...
for callout in callouts:
    ...
    losing_boards = controller.losing_boards()
    if len(losing_boards) == 1:
        for callout in callouts[callouts.index(callout):]:
            losing_boards[0].move(callout)
            if losing_boards[0].check_win():
                break
        losing_board_sum = losing_boards[0].find_sum()
        print(f'Losing Score: {losing_board_sum * int(callout)}')
        break